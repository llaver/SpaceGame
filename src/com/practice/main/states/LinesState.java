...
public class LinesState extends BasicGameState {
	...
	public void init(GameContainer container, StateBasedGame game) throws SlickException {
		...
		for(int i = 0; i < javaAwtPoints.size(); i++) {
			for(int j = 0; j < javaAwtPoints.size(); j++) {
				...
				Point pi = new Point((int) javaAwtPoints.get(i).getX(), (int) javaAwtPoints.get(i).getY(), col);
				Point pj = new Point((int) javaAwtPoints.get(j).getX(), (int) javaAwtPoints.get(j).getY(), col);
				allPoints.add(pi);
				allPoints.add(pj);
			}
		}
	...
	public LinkedList<Point> progressList(LinkedList<Point> list) {
		...
		for(int i = 1; i < list.size(); i++) {
			...
			temp.add(new Point((int) tempPoint.getX(),(int) tempPoint.getY(), list.get(i - 1).getColor()));
		}
		Point listPoint = list.get(0);
		temp.add(new Point((int) listPoint.getX(),(int) listPoint.getY(), list.get(list.size() - 1).getColor()));
		return temp;
	}
}
